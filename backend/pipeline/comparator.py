...
def get_comparator():
    ...
    if not api_key:
        logger.warning("GEMINI_API_KEY not found. Comparator will run in demo/offline mode.")
...
class MockComparator:
    def compare(self, report1: dict, report2: dict) -> dict:
        return {
            "improved": ["Hemoglobin increased by 1.2 g/dL, moving closer to normal range.", "Vitamin D improved slightly."],
            "declined": ["Fasting Glucose increased from 118 mg/dL to 125 mg/dL.", "LDL Cholesterol increased."],
            "next_steps": ["Focus strongly on reducing sugar intake.", "Continue iron supplements as they are working."]
        }
